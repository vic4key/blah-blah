    ...
    *yuv_size = width * height * 3 / 2; // YUV420 requires 3/2 the size of the original
    ...
    for (uint32_t y = 0; y < height; ++y)
    {
        for (uint32_t x = 0; x < width; ++x)
        {
            ...
            if (y % 2 == 0 && x % 2 == 0)
            {
                const uint32_t uv_idx = ((y / 2) * uv_width + (x / 2)) * 2;
                const uint8_t u_value = (uint8_t)(-0.147 * r - 0.289 * g + 0.436 * b + 128);
                const uint8_t v_value = (uint8_t)(0.615 * r - 0.515 * g - 0.100 * b + 128);
                yuv_data[width * height + uv_idx] = u_value;
                yuv_data[width * height + uv_idx + 1] = v_value;
            }
        }
    }
...
{
    if (size < width * height * 3 / 2 || rgba_size < width * height * 4 || !rgba_data)
    {
        return false;
    }
